...
This document describes how to write and use unit tests for VCOS. 
...
Please install the following dependencies to use unit testing:
...
1. Create the following dummy classes in `vcos/example`:
...
This example does not use any VCOS code, but if your unit tests do, then remember to update header stubs in `UNITTESTS/target_h` and source stubs in `UNITTESTS/stubs` with any missing type or function declarations. 

### Building and running unit tests

Run CMake and a Make program directly.
...
1. Run CMake with `-DCMAKE_BUILD_TYPE=Debug`, and then run a Make program.
