...
__attribute__((naked)) void hardFaultDefault(void)
{
    ...
    __asm__ volatile(".syntax unified                    \n"
                     ...
                     :
                     : [ sram ] "r"((uintptr_t)&_sram + HARDFAULT_HANDLER_REQUIRED_STACK_SPACE), [ eram ] "r"(&_eram),
                       [ estack ] "r"(&_estack)
                     : "r0", "r4", "r5", "r6", "r8", "r9", "r10", "r11", "lr");
}

__attribute__((used)) void hard_fault_handler(uint32_t *sp,
                                              uint32_t  corrupted,
                                              uint32_t  exc_return,
...
ISR_VECTOR(0)
const cortexmBase cortexVectorBase = {&_estack,
                                      {
