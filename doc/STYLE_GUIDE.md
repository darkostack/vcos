# VCOS (Vertexcom-OS) Coding Conventions and Style
...
- C
  - VCOS uses and enforces the ISO9899:1999 (aka ISO C99, C99) C language standard as the minimum.
- C++
  - VCOS uses and enforces the ISO14882:2003 (aka ISO C++03, C++03) C++ language standard as the minimum.
- Extensions
...
- C Preprocessor
  ...
  - Preprocessor `#include` directives should be grouped, ordered, or sorted as follows:
    - If the unit is a core/private header file, `"vcos-core-config.h"` should be the first header file included.
    - If the unit is a core/private `.c` or `.cpp` file:
      ...
      - If the unit has no corresponding header file, then it should directly include `"vcos-core-config.h"` before any other header file.
...
- VCOS uses the `make pretty` build target to reformat code and enforce code format and style.  The `make pretty-check` build target is included in VCOS's continuous integration (TODO) and must pass before a pull request is merged.
...
- Case
  ...
  - All VCOS class, namespace, structure, method, function, enumeration, and type names in the C/C++ language shall be in *upper camel case*.  Exception: the top level VCOS namespace 'vc'.
  - All VCOS instantiated names of instances of classes, namespaces, structures, methods, functions, enumerations, and types as well as method and function parameters in the C++ language shall be in *lower camel case*.
- Symbol Qualification
  - All VCOS C public data types and free functions should have `vc` prepended to their name.
  - All VCOS C++ code should be in the ‘vc’ top-level namespace.
- Scope
...
- VCOS uses and enforces both Python 2 and Python 3.  Support for Python 2 is a result of the fact that some current Linux distributions and Macs are still using 2.x as default.
